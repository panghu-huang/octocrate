...
use octocrate_infra::{
  ExpirableToken, GithubAPIClient, GithubAPIConfig, GithubAPIRequest, GithubError, GithubResult,
};
use std::{
  collections::HashMap,
...
impl GithubApp {
  ...
  pub fn get_installations(&self) -> GithubAPIRequest<Vec<GithubInstallation>> {
    self
      .api_client
...
impl GithubAppBuilder {
  ...
  pub fn build(self) -> GithubResult<GithubApp> {
    let app_id = self
      .app_id
      .ok_or_else(|| GithubError::new("app_id is required to create a GithubApp".to_string()))?;
...
mod tests {
  ...
  async fn get_installations() -> GithubResult<()> {
    ...
    let _installations = app.get_installations().send().await?;
